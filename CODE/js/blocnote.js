...
$(document).ready(function() {
    removeNote(); // Initialiser la fonction pour les événements de clic
});
...
function favouriteNote() {
  $(".favourite-note").off('click').on('click', function(event) {
      ...
      var noteItem = $(this).parents('.single-note-item');
      noteItem.toggleClass('note-favourite');
      
      var noteId = $(this).siblings('.remove-note').data('note-id');
      var isFavourite = noteItem.hasClass('note-favourite') ? 1 : 0;

      // Changer l'icône de l'étoile en fonction du statut de favori
      $(this).toggleClass('fa-solid', isFavourite);
      $(this).toggleClass('fa-regular', !isFavourite);

      // Envoyer la requête AJAX pour mettre à jour la base de données
      $.ajax({
          url: 'update_fav.php',
          method: 'POST',
          data: { id: noteId, fav: isFavourite },
